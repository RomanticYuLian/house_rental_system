<template>
  <div>
    ...
    <div class="main">
      <!-- 查询 -->
      <keep-alive>
        <el-input placeholder="请输入房主或租户进行查询" size="small" v-model="keyword" @input="handleQuery">
        </el-input>
      </keep-alive>
      <!-- 表格 -->
      <el-table style="width: 100%" :data="orderManageData">
        ...
        <el-table-column prop="lesseeName" label="租户">
        </el-table-column>
...
export default {
  ...
  mounted() {
    this.gainOrderManageData();
  },
  computed: {
    ...mapGetters(["orderManageData", "orderManageTotal"]),
  },
  methods: {
    handleQuery() {
      this.gainOrderManageData();
    ...
    gainOrderManageData() {
      this.$store.dispatch("manage/getOrderManageData", {
        pn: this.currentPage,
        size: this.pagesize,
